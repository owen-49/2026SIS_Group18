...
Manifest V3 extension that detects BibTeX and `\\cite{...}` commands in Overleaf. It opens a searchable bibliography in Chrome's Side Panel and sends the `.bib` metadata to the local ClaimTrace backend for bibliography Audit.
...
The extension starts with four demo sources. When it can read BibTeX from the active Overleaf editor, the detected `.bib` content is uploaded to `http://localhost:8000/api/parse`, then checked with `POST /api/audit`. The extension does not call `/api/verify`; claim verification is outside the plugin scope.
...
Audit is the only backend workflow implemented by the extension. A detected BibTeX file is
...
records, and lookup failures. They do not claim that a manuscript sentence is
...
entry, and its location in the editor. The extension does not fetch claim
evidence or call `/api/verify`. It does not show a spinner or an empty quote
while the separate bibliography Audit is running.
...
hovering a citation requires. It is a local citation-location preview; it is
not a claim Verify result.
...
only if the backend returns 404. This is the plugin's only backend workflow.
