...
The `/verify` screen is analysis-driven: it selects an uploaded manuscript, shows its extracted text on the left, loads claim-and-citation pairs from `GET /api/papers/{paper_id}/claims`, and shows persisted BibTeX/source-PDF resolution on the right. An identified cited source is automatic; a missing citation may return optional `similar_sources` candidates that the user can choose for comparison without treating the candidate as the original citation. The `/audit` screen sends the selected manuscript and completed source PDFs to `POST /api/audit`, then renders the returned manuscript, evidence passages, locations, and verdicts. Real API mode shows an explicit unavailable or pending state when persisted analysis data is not ready.
